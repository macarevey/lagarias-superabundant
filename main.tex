...
\begin{proof}
Suppose, for the sake of contradiction, that $m$ is the smallest counterexample to \eqref{eq:Lagarias} and that $m$ is not superabundant.
Let $n<m$ be the greatest superabundant number less than $m$. Since $m$ is not superabundant, there exists $k<m$ with
$\frac{\sigma(k)}{k}\ge \frac{\sigma(m)}{m}$; taking $k$ with maximal $\frac{\sigma(k)}{k}$ among $1\le j<m$ makes $k$ superabundant, hence $k\le n$, so
$\frac{\sigma(n)}{n}\ge \frac{\sigma(m)}{m}$.
Thus
\[
\frac{\sigma(n)}{n}\ \ge\ \frac{\sigma(m)}{m}\ >\ B_m\ >\ B_n,
\]
so $n<m$ is also a counterexample, contradicting the minimality of $m$. Hence $m$ must be superabundant.
\end{proof}
